...
function markReadyUI() {
  const el = document.querySelector('#fbReady');
  if (el) {
    el.textContent = 'Firebase siap';
...
try {
  ...
  // update UI + broadcast event
  markReadyUI();
  window.dispatchEvent(new Event("firebase-ready"));
  console.log("[WEDSYS] Firebase ready (boot v4)");

  // muat logic dashboard (biar kalau gagal pun UI sudah 'siap')
  import("/vendor.js?v=47").catch(e => console.error("Gagal import vendor.js", e));
